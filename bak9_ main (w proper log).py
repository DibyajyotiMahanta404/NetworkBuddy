...
def _parse_customer_query(text: str) -> Dict[str, Any]:
    ...
    m = re.search(r"\bregion[:=\s]+([A-Za-z]+)\b", tl)
    if not m:
...
def orchestrator_agent(state: AgentState) -> AgentState:
    ...
    if s["intent"] in {"performance", "fault", "inventory", "data_flow", "report"} and (
        s.get("customer_id") or s.get("customer_region")
    ):
        if not s.get("target_nodes"):
            try:
                mapped, reason = _resolve_targets_from_customer_context(s)  # defined elsewhere in your project
                if mapped:
    ...
    s["customer_needs_enrichment"] = (
        s["intent"] in {"fault", "performance", "inventory", "data_flow", "report"}
    ...
    if s["intent"] == "customer":
        ...
        if s.get("_mapping_used") and include_inv and primary in {"performance_agent", "fault_agent", "report"}:
    ...
    try:
        ...
        if open_list:
            s["ticketing_summary"] = _format_tickets_list(open_list, "Open tickets for requested nodes")
            emit_flow_event("open_tickets_found", count=len(open_list))
            return s
